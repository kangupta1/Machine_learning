...
class WeatherForecast:
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        day_over_day_change = avg_per_day[1:] - avg_per_day[:-1]
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        _, extreme_per_day_indices_ = torch.max(diffs, dim=1)
        return self.data.gather(1, extreme_per_day_indices_.view(-1, 1)).squeeze()
    ...
    def max_last_k_days(self, k: int) -> torch.Tensor:
        ...
        max_per_day, _ = torch.max(last_k_days, dim=1)
        return max_per_day
